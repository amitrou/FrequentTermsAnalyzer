  ...
  private function CandidatesCleanUp($firstResult, $termLength){
    ...
    for ($i=$firstResult; $i<=$termLength; $i++){
      foreach($this->termCandidates[$i] as $candidate=>$termCounter){
        ...
        if ( $firstResult==1 || (!isset($this->excludedWords[$terms[0]])&& !isset($this->excludedWords[$terms[count($terms)-1]])) ) $out[$candidate] = round($termCounter / $this->termsArraySize, 4);
      }
    }
    arsort($out);
    return $out;
